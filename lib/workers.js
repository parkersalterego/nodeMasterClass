...
workers.validateCheckData = (originalCheckData) => {
  ...
  if (
    ...
    debug('Error: One of the checks is not properly formatted. Skipping check.');
  }
...
workers.rotateLogs = () => {
  ...
  _logs.list(false, (err, logs) => { // _logs.list boolean argument determines wether or not to incllude compressed files
          ...
          if (!err) {
            ...
            _logs.truncate(logId, (err) => {
              if (!err) {
                ...
                debug('Error: Unable to truncate log file');
              }
            });
          } else {
            debug('Error: unable to compress one of the log files');
          }
        });
      });
    } else {
...
workers.init = () => {
  ...
  workers.rotateLogs();
  // call the compression loop to compress logs while server is running
  workers.logRotationLoop();
};
